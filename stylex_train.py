...
def reconstruction_loss(encoder_batch: torch.Tensor, generated_images: torch.Tensor, generated_images_w: torch.Tensor,
    ...
    encoder_batch_norm = encoder_batch_norm[:, :3, :, :]
    generated_images_norm = lpips_normalize(generated_images)
    generated_images_norm = generated_images_norm[:, :3, :, :]
...
class Trainer():
    def __init__(
            ...
            kl_scaling=1,
            rec_scaling=10,
            classifier_path="",  # TODO: Used to be FFHQ-Gender.pth,
    ...
    def set_data_src(self, folder='/code/combinepng', dataset_name=None):
    ...
    def train(self):
        ...
        wandb.log({'loss/G': self.g_loss, 'steps': self.steps})
        wandb.log({'loss/D': self.d_loss, 'steps': self.steps})
        wandb.log({'loss/rec': self.total_rec_loss, 'steps': self.steps})
        wandb.log({'loss/kl': self.total_kl_loss, 'steps': self.steps})
        wandb.log({'FID': self.last_fid, 'steps': self.steps})
        self.steps += 1
        self.av = None
